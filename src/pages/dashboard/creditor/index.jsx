...
const CreditorsList = () => {
  ...
  const { inputs, handleSubmit, handleChange, errors, setFieldValue, reset } =
    useForm({
      defaultValues: { session: "", type: "creditors", term: "First Term" },
      validation: {
        session: {
          required: true,
        },
  ...
  const data = {
    creditors: incomeCreditors,
  };

  const title = {
    creditors: "Creditor List",
  };

  const commonGroupButtonOptions = [
    {
      title: "Extract Creditor",
      type: "button",
      ...
      >
        <div className="form-group mb-4">
          <AuthSelect
            ...
            options={[
              { value: "creditors", title: "Creditors List" },
            ]}
          />
